...
def handler_dispatcher(start='12/28/2020'):  # city_dep, city_arr #datetime.datetime.today()
    ...
    # общий принцип понятен, я не понимаю как это все реализовать вот у меня есть 5 рейсов Москва - Санкт-Петербург
    # я не понимаю как в список добавить рейсы Москва-Казань с другой периодичностью.
    # TODO Периодичность можно реализовать через timedelta
    # TODO плюс можно ещё calendar использовать (тоже в снипетах есть)
    # TODO что-то такое будет
    # TODO цикл по одному списку городов
    # TODO     цикл по другому списку городов
    # TODO         генерация списка дат от текущей
    # TODO         добавление в словарь (или сразу запись в json)
    # Вы пишите, что данные по рейсам надо записывать
    # в json файл, а как его создать в информации которую я нашла в уже готовый json-файл идет запись.
    # TODO в лекциях, в сниппетах есть создание Json файла и его наполнение с 0
    for day_kzn in week:
        dict1 = {'Москва': {
            'Казань': [day_kzn]}}
