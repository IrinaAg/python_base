...
def woldwide(k, v, analized_res):
    ...
    # Перебором получается портянка, но по другому не понимаю как это реализовать можете какие-то примеры показать?
    if 'X' in v:
        if 'X' in analized_res[k + 1]:
            if 'X' in analized_res[k + 2]:
...
def add_points(throw=2, frames=frame, index=0):
...
# индекс_внутри_фрейма = 0  -- чтобы знать берем мы первый элемент во фрейме, или второй
    ...
    if frame
    # если есть следующий фрейм (проверяем есть ли в списке индекс после изменения, либо опять проверяем текущий, если индекс не менялся)
