...
# TODO     если фрейм[0] равен страйку:
# TODO         результат += 10  -- очки за текущие сбитые кегли
# TODO         результат += функция_получения_доп_очков(за 2 броска, из списка фреймы, начиная с текущего индекса)
# TODO                      эту функцию мы напишем после, пока предположим, что она просто возвращает сбитые кегли за 2 след броска
...
# TODO функция_получения_доп_очков(броски=2, список=фреймы, индекс=0)
# TODO внутри мы выполняем действия
# TODO     доп_очки = 0
# TODO     индекс_внутри_фрейма = 0  -- чтобы знать берем мы первый элемент во фрейме, или второй
# TODO     если есть следующий фрейм (проверяем есть ли в списке индекс)
# TODO         если первый бросок там - число
# TODO             доп_очки += int(число)
# TODO             бросок -= 1
# TODO             индекс_внутри_фрейма = 1  -- переключаем индекс внутри на следующий бросок
# TODO         если бросок - страйк
# TODO             доп_очки += 10
# TODO             бросок -= 1
# TODO             индекс_внутри_фрейма = 0 -- после страйка мы будем брать 0ой элемент из следующего фрейма
# TODO             индекс += 1 -- чтобы переместить указатель на следующий фрейм в списке фреймы
# TODO         если бросок - "-"
# TODO             бросок -= 1
# TODO             индекс_внутри_фрейма = 1
# TODO     если есть следующий фрейм (проверяем есть ли в списке индекс после изменения, либо опять проверяем текущий, если индекс не менялся)
# TODO         если второй бросок - число
# TODO             доп_очки += int(число)
# TODO         если второй бросок - спэр
# TODO             доп_очки = 10
# TODO         если второй бросок - страйк
# TODO             доп_очки += 10
# TODO ну и осталось написать return доп_очки